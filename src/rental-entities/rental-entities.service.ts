...
export class RentalEntitiesService {
  ...
  async syncCommand(url: string): Promise<any> {
      return new Promise( resolve => {
        this.httpService
            .get(url).subscribe(async r => {
          await r.data.map(async (createRentalEntityDto: CreateRentalEntityDto) => {
            await this.httpService.post(`${this.environmentConfigService.getServiceEndpoint()}/rental-entities/queue`,
  ...
  async sendEmail(address: string){
    let info = await this.emailTransporter.sendMail({
