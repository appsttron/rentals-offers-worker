...
export class RentalEntitiesQueueController {
  ...
  async syncCommand(@Payload() data: any, @Ctx() context: RmqContext) {
    const result = await this.rentalEntityService.syncCommand(
      (JSON.parse(data) as { url: string }).url
    );
  ...
  processRentalEntity(@Payload() data: any, @Ctx() context: RmqContext) {
    this.rentalEntityService
      .processRentalEntity(JSON.parse(data) as CreateRentalEntityDto)
      .then(() => {
        const channel = context.getChannelRef();
        const originalMsg = context.getMessage();
        channel.ack(originalMsg);
